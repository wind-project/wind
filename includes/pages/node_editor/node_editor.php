...
class node_editor {
  ...
  function table_ip_ranges_v6() {
	...
	function table_subnets_v6() {
		global $construct, $db, $lang;
		$table_subnets_v6 = new table(array('TABLE_NAME' => 'table_subnets_v6', 'FORM_NAME' => 'table_subnets_v6'));
		$table_subnets_v6->db_data(
			'subnets_v6.id, "" AS subnet_v6, subnets_v6.v6net, subnets_v6.ipv6_end, subnets_v6.type, "" AS peer, n_l.name AS link_node_name, n_l.id AS link_node_id, n_c.name AS client_node_name, n_c.id AS client_node_id, links.type AS links_type, links.ssid AS links_ssid',
			'subnets_v6
			LEFT JOIN links ON links.id = subnets_v6.link_id
			LEFT JOIN nodes AS n_l ON n_l.id = links.peer_node_id
			LEFT JOIN nodes AS n_c ON n_c.id = client_node_id',
			'subnets_v6.node_id = '.intval(get('node')),
			"",
			"subnets_v6.type ASC, subnets_v6.v6net ASC");
		foreach( (array) $table_subnets_v6->data as $key => $value) {
			if ($key != 0) {
				$table_subnets_v6->data[$key]['v6net'] = @inet_ntop($table_subnets_v6->data[$key]['v6net']);
				$table_subnets_v6->data[$key]['ipv6_end'] = @inet_ntop($table_subnets_v6->data[$key]['ipv6_end']);
				$table_subnets_v6->data[$key]['subnet_v6'] = $table_subnets_v6->data[$key]['v6net']." - ".$table_subnets_v6->data[$key]['ipv6_end'];
				if ($table_subnets_v6->data[$key]['type'] == 'link') {
					if($table_subnets_v6->data[$key]['links_type'] == 'p2p')
						$table_subnets_v6->data[$key]['peer'] = $table_subnets_v6->data[$key]['link_node_name']." (#".$table_subnets_v6->data[$key]['link_node_id'].")";
					elseif ($table_subnets_v6->data[$key]['links_type'] == 'ap')
						$table_subnets_v6->data[$key]['peer'] = $lang['ap']." - ".$table_subnets_v6->data[$key]['links_ssid'];
				}
				if ($table_subnets_v6->data[$key]['type'] == 'client') {
					$table_subnets_v6->data[$key]['peer'] = $table_subnets_v6->data[$key]['client_node_name']." (#".$table_subnets_v6->data[$key]['client_node_id'].")";
				}
			}
		}
		$table_subnets_v6->db_data_multichoice('subnets_v6', 'id');
		for($i=1;$i<count($table_subnets_v6->data);$i++) {
			if (isset($table_subnets_v6->data[$i])) {
				$table_subnets_v6->info['EDIT'][$i] = make_ref('/node_editor/subnet_v6', array('node' => intval(get('node')), "subnet_v6" => $table_subnets_v6->data[$i]['id']));
			}
		}
		$table_subnets_v6->info['EDIT_COLUMN'] = 'subnet_v6';
		$table_subnets_v6->info['MULTICHOICE_LABEL'] = 'delete';
		$table_subnets_v6->db_data_remove('id', 'v6net', 'ipv6_end', 'link_node_name', 'link_node_id', 'client_node_name', 'client_node_id','links_type','links_ssid');
		$table_subnets_v6->db_data_translate('subnets_v6__type');
		return $table_subnets_v6;
	}
	...
	function output() {
		...
		if (get('action') == 'delete') {
			...
			if (get('node') != 'add') {
				...
				$this->tpl['table_subnets_v6'] = $construct->table($this->table_subnets_v6(), __FILE__);
				...
				$this->tpl['link_subnet_v6_add'] = make_ref('/node_editor/subnet_v6', array('node' => get('node'), 'subnet_v6' => 'add'));
