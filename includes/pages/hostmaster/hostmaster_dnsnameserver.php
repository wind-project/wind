...
class hostmaster_dnsnameserver {
	...
	function form_nameserver() {
		...
		$form_nameserver->data[1]['value'] = long2ip($form_nameserver->data[1]['value']);
                $form_nameserver->data[2]['value'] = varbinary2ipv6number($form_nameserver->data[2]['value']);
	...
	function output() {
		...
		if(get('action') === "delete")
		{
			$ret = $db->del("dns_nameservers, dns_zones_nameservers", 
					...
					"dns_nameservers.id = '".get('nameserver')."'");
			if ($ret) {
				$main->message->set_fromlang('info', 'delete_success', make_ref('/hostmaster/dnsnameservers'));
			} else {
		...
		$this->tpl['link_nameserver_delete'] = self_ref(array("action" => "delete"));
	...
	function output_onpost_form_nameserver() {
		...
		$_POST['dns_nameservers__ip'] = ip2long($_POST['dns_nameservers__ip']);
                $_POST['dns_nameservers__ipv6'] = ipv6number2varbinary($_POST['dns_nameservers__ipv6']);
		$ret = $form_nameserver->db_set(array(),
							"dns_nameservers", "id", get('nameserver'));
		
		if ($ret) {
			$main->message->set_fromlang('info', 'edit_success', make_ref('/hostmaster/dnsnameservers'));
		} else {
			$main->message->set_fromlang('error', 'generic');		
		}
	}
