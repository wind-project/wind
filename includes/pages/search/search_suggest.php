	...
	function output() {
		...
		if (is_numeric($q) && strpos($q, ".") === FALSE) {
			...
			foreach ((array)$this->tpl['nodes_search'] as $key => $value) {
				$this->tpl['nodes_search'][$key]['href'] = makelink2("/nodes", array("node" => $this->tpl['nodes_search'][$key]['id']));
			}
			...
			foreach ((array)$this->tpl['ip_search'] as $key => $value) {
				...
				$this->tpl['ip_search'][$key]['href'] = makelink2("/nodes", array("node" => $this->tpl['ip_search'][$key]['id']));
			}
			...
			foreach ((array)$this->tpl['dns_search'] as $key => $value) {
				...
				$this->tpl['dns_search'][$key]['href'] = makelink2('/nodes', array("node" => $this->tpl['dns_search'][$key]['id']));
			}
		} else {
			$this->tpl['nodes_search'] = $db->get(
										'nodes.id, nodes.name',
